...
=================================
  All secrets fetched from GCS Secret Manager at runtime.
...
def validate_prereg(prereg: dict) -> list:
    ...
    if "salt_hash" in prereg and prereg["salt_hash"] != sha256s(SALT):
        problems.append("salt_hash does not match audit salt")
    return problems
...
def main():
    global ENGINE_BIN, ENGINE_KEY, SALT, SEED, SALT_SOURCE
    ...
    if args.verify:
        ok = verify_run(args.verify, salt=SALT if SALT else "")
    ...
    ENGINE_BIN   = fetch_secret(gcs_project, 'sov-engine-bin')
    ENGINE_KEY   = fetch_secret(gcs_project, 'sov-engine-key')
    SALT         = fetch_secret(gcs_project, 'sov-audit-salt')
    SEED         = hashlib.sha256(b'tsa-harness-salt:' + SALT.encode()).digest()
    SALT_SOURCE  = 'gcs:sov-audit-salt'
